...
**コンセプト**: 完全ブラウザ完結・永続無料・依存最小のAI付きIDE。Transformers.js v4（ONNX Runtime Web + WebGPU）でブラウザ内ローカル推論を行い、コードもデータも外部に出ない。
...
[AI Inference]
  @huggingface/transformers (v4+)  # ブラウザ内LLM推論エンジン
                                   # ONNX Runtime Web (WebGPU + WASM 両対応)
                                   # 約200アーキテクチャ、8B+ LLM、Mamba/MoE/MLA対応
  Web Worker                       # メインスレッドブロック防止
  モデル候補（onnx-community/ ホスト、軸×サイズの3層構造で7+2候補）:
    高性能 5〜6GB帯（VRAM 8GB+）:
      汎用       : gemma-4-E4B-it-ONNX (~4GB, マルチモーダル, 256Kコンテキスト, Apache 2.0)
                   Qwen3-8B-ONNX (~5〜6GB, Apache 2.0)
      コーディング: Qwen2.5-Coder-7B-Instruct-ONNX (~5GB, FIM対応, HumanEval 88.4%, Apache 2.0)
      推論       : DeepSeek-R1-Distill-Qwen-7B-ONNX (~5GB, MATH-500 92.8%, MIT)
    中性能 2〜4GB帯（VRAM 4〜8GB）:
      汎用       : Qwen3-4B-Instruct-2507-ONNX (~3〜4GB, 思考モード, 256K, Apache 2.0)
      コーディング: Qwen2.5-Coder-3B-Instruct-ONNX (~2GB, FIM対応, Apache 2.0)
      推論       : Phi-4-Mini-ONNX (~3GB, MIT, Microsoft 2025)
    低性能 ~1.5GB帯（CPUフォールバック / モバイル）:
      Qwen2.5-Coder-1.5B-Instruct-ONNX | Llama-3.2-1B-Instruct (~0.6GB)
  ※ コード補完UI（grey候補・Tab確定）は FIM 学習済みの Qwen2.5-Coder 系でのみ実装可能
  ※ Gemma 4 E4B はテキスト・画像・音声マルチモーダル、140+言語対応
  ※ Qwen3-Coder 系（30B MoE / 480B）はブラウザ非現実的、外部API待ち
...
- @mlc-ai/web-llm → 初期採用後、Transformers.js v4 (2026-02) 登場で乗り換え。
                    理由: 新モデル対応がMLCコンパイラ待ちで遅く、Gemma 4・Qwen3-Coder系が
                    入らない。v4は ONNX Runtime + WebGPU で約200アーキテクチャ網羅、
                    8B+ LLM対応、新モデル即追従、CPU fallback も同一APIで動く。
```

### 推論性能の現実的な上限

- ブラウザWebGPUで実用的に動くモデル: 〜8B程度（q4f16量子化込み、Transformers.js v4 のC++ランタイムでBERTなら従来比約4倍高速）
- ハイエンドPC（VRAM 16GB+）でも実用速度を考えると8B〜13Bが実質上限
- 26B/30B以上の大型モデル（Qwen3-Coder 30B MoE、Gemma 4 26B等）は諦める方針
- 「クラウドAPIや外部Ollamaなら使える大型モデル」は本プロジェクトのスコープ外
- 思想を一貫させるため、推論性能の天井は WebGPU + Transformers.js v4 (ONNX Runtime Web) に従う
...
[ブラウザ内（すべてローカル）]
  ...
  ├─────────────────────────────────┤
  │ AI Worker（Transformers.js v4 + ONNX）│
  ├─────────────────────────────────┤
  ...
  └─────────────────────────────────┘
       ↑              ↑              ↑
  Cloudflare Pages  HuggingFace CDN  GitHub
...
- [ ] チャット欄に質問すると Transformers.js v4（Gemma 4 / Qwen3 系 ONNX）が答える
...
Day 5-6:   Transformers.js v4 を Web Worker で動かす、最小チャットUI
...
**最重要リスク**: Transformers.js v4 + WebGPU が手元のマシンで実用速度（コーディング用途で 8〜15 tok/s 程度）で動くか。Phase 1完成後に必ずベンチマーク。遅すぎたらモデル選定（Gemma 4 E4B → E2B → Qwen2.5-Coder-1.5B 等）からやり直し。
