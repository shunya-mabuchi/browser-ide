...
- LLM ゲート画面廃止：起動直後からエディタ完全動作、モデルロードはバックグラウンド
- WebGPU 検出 + WASM (CPU) フォールバックの正式対応
...
| 起動シーケンス | エディタ即時表示、モデルロードはバックグラウンド、ChatPanel 内に状態統合 | 全画面ゲート方式は「ダウンロード待ち何もできない」UX 破綻、モデル切替もしにくい |
| LLM 推論バックエンド | WebGPU 優先、未対応環境は WASM (CPU) 自動フォールバック + 速度警告 | WebGPU 固定はバグ（現状）、CPU 固定は速度致命的、`'auto'` は失敗検知できず警告出せない |
...
## 6. 起動シーケンスと LLM ライフサイクル

現状（commit `7ec4ff3`）の `App.tsx` は `AppState = 'model-select' | 'ready'` でモデル選択を全画面ゲートにしている。これを廃止して、エディタは即時表示・モデルロードはバックグラウンド方式に変更する。
...
段階 1（即時、〜200ms）: 
  - OPFS 初期化
  - WorkspaceStore ツリー読み込み（キャッシュ済み or seed）
  - TabManager セッション復元（.browser-ide/session.json）
  - 4 ペインレイアウト描画
  ↓
[エディタ完全動作開始] ← この時点で以下すべて利用可:
  ファイル CRUD / タブ切替 / 編集 / Preview 実行 / コンソール

  ↓ 並行してバックグラウンドで:

段階 2（並行・〜500ms〜数分）:
  - detectHardware() で WebGPU / GPU tier 判定
  - localStorage.bide.lastModel から前回モデル ID を取得
  - 値あり: そのモデル ID + 検出した device で LLM Worker 自動ロード
    - HuggingFace ファイルが Cache API にキャッシュ済みなら数秒
    - 未キャッシュなら通常ダウンロード（数分）
  - 値なし: ChatPanel が「モデルを選択してください」状態を表示

段階 3（モデルロード完了後）:
  - ChatPanel が利用可能状態に遷移
  - chat 入力欄が活性化
```
...
```
[idle] モデル未選択（初回起動 or 履歴クリア後）
  └─ UI: 「AI を使うには[モデルを選択]ボタン」
      クリック → ModelSelector モーダル表示 → 選択 → loading に遷移

[loading] モデルダウンロード中
  └─ UI: モデル名 + プログレスバー + 「キャンセル」
      ※ エディタ・実行・ファイル操作は完全動作
      完了 → ready 遷移 / キャンセル → idle 遷移 / 失敗 → error 遷移

[ready] モデル利用可
  └─ UI: 通常の Chat（messages + 入力欄 + 送信）
      タイトル右の「⋯」メニューから「モデル切替」「モデルアンロード」等

[error] ロード失敗
  └─ UI: エラーメッセージ + 「リトライ」「別のモデルを選択」
      原因: WebGPU 非対応 / モデル ID 404 / ネットワーク失敗 等
```
...
[ready] 状態で「⋯」→「モデル切替」
  ↓
ModelSelector がモーダル表示（全画面ゲートではなく、編集中のレイアウト維持）
  ↓
別モデル選択
  ↓
既存 Worker terminate → 新規 Worker 起動 → loading 状態
  ↓
ダウンロード中もエディタは完全動作
  ↓
完了 → ready 状態（新モデル名で動作）
```

### ModelSelector の責務変更

- **現状**: 全画面占拠の起動ゲート
- **変更後**: モーダル内のコンテンツ。`onSelect(modelId)` コールバックで親に通知して閉じる
- ハードウェア tier ベースの推奨表示は維持
- 「キャンセル」ボタンを追加（[idle] 状態に戻る）
...
```ts
// llm.worker.ts の WorkerMessage 型
type WorkerMessage =
  | { type: 'load'; modelId: string; dtype?: Dtype; device?: 'webgpu' | 'wasm' }
  | { type: 'chat'; messages: ChatMessage[] }
  | { type: 'abort' }

// load handler
generator = await pipeline('text-generation', msg.modelId, {
  device: msg.device ?? 'webgpu',
  dtype: msg.dtype ?? defaultDtypeFor(msg.device ?? 'webgpu'),
  progress_callback: handleProgress,
})

function defaultDtypeFor(device: 'webgpu' | 'wasm'): Dtype {
  return device === 'webgpu' ? 'q4f16' : 'q4'
}
```

呼び出し側（`App.tsx` または `useLlmModel` フック）:

```ts
const hw = await detectHardware()
const device = hw.hasWebGPU ? 'webgpu' : 'wasm'

if (!hw.hasWebGPU) {
  showToast('WebGPU 非対応環境です。CPU での動作になり推論が大幅に遅くなります（〜2 tok/s）。')
}
...
device によって最適な量子化が違う:

| device | 推奨 dtype | 備考 |
|---|---|---|
| `webgpu` | `q4f16` | f16 演算が高速、精度も十分 |
| `wasm` | `q4` または `int8` | f16 が遅い、整数演算優先 |

`hardware.ts` の `ModelOption.dtype` は現状 `q4f16` 固定だが、`device === 'wasm'` のときは `q4` にオーバーライドする処理を Worker 呼び出し直前に追加。
...
| キー | 用途 |
|---|---|
| `bide.lastModel` | 最後にロード成功したモデル ID。次回起動時の自動ロード対象 |
| `bide.autoLoadModel` | 自動ロードを有効/無効にするフラグ（既定: true）。「⋯」メニューの「次回も自動ロード」チェックボックスで切替 |

`bide.lastModel` は `done` メッセージ受信時のみ書き込む（失敗・キャンセル時は更新しない）。
...
- Worker terminate
- `generator = null` にリセット
- ChatPanel を [idle] 状態に戻す
- VRAM/RAM が解放される（特に CPU 動作時の RAM 圧迫を解消）

ユースケース: 「モデル切替前に一旦解放したい」「会議中に GPU を別アプリで使いたい」など。

### エラーハンドリング

| エラーケース | 表示 | リカバリ |
|---|---|---|
| `navigator.gpu` 未対応 | トースト警告 + WASM フォールバック自動 | 透過的、ユーザー操作不要 |
| WebGPU 対応だが GPU メモリ不足 | エラー: 「モデルが GPU に乗り切りません」 | 「より小さいモデルを選ぶ」リンク → ModelSelector |
| HuggingFace モデル ID 404 | エラー: 「モデルが見つかりません」 | 「別のモデルを選ぶ」リンク + GitHub Issue 報告ボタン |
| ネットワークエラー | エラー: 「接続できません」 | 「再試行」ボタン |
| OPFS Quota Exceeded | トースト | キャッシュクリア手順をモーダルで案内 |

### 並列処理の確認

```
時刻 T0:    ページロード
時刻 T0+200ms: エディタ表示完了、ユーザー操作可能
時刻 T0+500ms: ハードウェア検出完了、Worker 起動開始
時刻 T0+5min:  モデルダウンロード完了、Chat 利用可

ユーザー視点:
  T0+200ms 〜 T0+5min の間も:
  - コード編集できる
  - 実行・プレビューできる
  - ファイル CRUD できる
  - DnD 並び替えできる
...
LLM Worker（WebGPU 推論）と Bundler Worker（CPU + esbuild-wasm）は別 Worker かつリソース競合しない（GPU vs CPU）。実測でも問題なく並列動作することを Phase 1 完了時に検証する。
...
- [ ] 起動時 seed
- [ ] LLM ゲート画面廃止（エディタ即時表示、モデルロードはバックグラウンド）
- [ ] ChatPanel の 4 状態 UI（idle / loading / ready / error）
- [ ] ModelSelector のモーダル化（モデル切替・キャンセル対応）
- [ ] `localStorage.bide.lastModel` による前回モデル自動復元
- [ ] `llm.worker.ts` の `device` 引数化（`'webgpu' | 'wasm'`）
- [ ] `detectHardware()` 結果に応じた device 自動選択 + dtype オーバーライド
- [ ] WebGPU 非対応環境での速度警告トースト
- [ ] モデルアンロード機能（「⋯」メニュー）
- [ ] 自動保存（800ms debounce）
